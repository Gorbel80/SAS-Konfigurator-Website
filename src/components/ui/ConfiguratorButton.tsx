...
const sizes: Record<Size, { btn: string; icon: string; badge: string }> = {
  hero: {
    btn: "h-10 gap-2 rounded-full px-3.5 pl-3 text-sm sm:h-11 sm:px-4 sm:text-[0.9375rem]",
    icon: "h-4 w-4",
    badge: "px-2 py-0.5 text-[9px] tracking-[0.1em]",
  },
  header: {
    btn: "h-8 gap-1.5 rounded-full px-2.5 pl-2 text-xs sm:text-[13px]",
    icon: "h-3.5 w-3.5",
    badge: "px-1.5 py-px text-[8px] tracking-[0.08em]",
  },
  mobile: {
    btn: "h-10 w-full gap-2 rounded-full px-4 text-sm",
    icon: "h-4 w-4",
    badge: "px-2 py-0.5 text-[9px] tracking-[0.1em]",
  },
};
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        "bg-accent",
        "shadow-[0_1px_0_rgba(255,255,255,0.18)_inset,0_4px_14px_-4px_rgba(11,107,203,0.45)]",
        "ring-1 ring-accent/30",
        "transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)]",
        "hover:bg-accent-hover hover:shadow-[0_1px_0_rgba(255,255,255,0.2)_inset,0_6px_18px_-4px_rgba(11,107,203,0.5)]",
        s.btn,
        className,
      )}
    >
