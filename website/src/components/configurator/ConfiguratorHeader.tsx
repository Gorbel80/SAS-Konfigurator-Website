...
import { getWebsiteHomeUrl } from "@/lib/urls";
 ...
 */
export function ConfiguratorHeader({ titleWord, backLabel, locale }: Props) {
  const websiteHome = getWebsiteHomeUrl(locale);
  ...
  return (
    <header className="z-30 shrink-0 border-b border-black/40 bg-[#0d0f12]">
      ...
      <div className="flex h-10 items-center gap-1.5 overflow-x-auto border-t border-white/5 bg-[#12151a] px-2.5 custom-scroll sm:px-3 lg:px-4">
        ...
        />
        <ToolbarBtn label="CAD" icon={FileOutput} className="lg:hidden" />
